...
contains
  ...
  subroutine SoilThermProp (bounds,  num_nolakec, filter_nolakec, &
    ...
    real(r8) :: zh2osfc
    ! Finn Windfly for local variables defination
    real(r8) :: mu1
    real(r8) :: mu2
    !-----------------------------------------------------------------------
         ...
         do fc = 1,num_nolakec
            ...
            if ((col%itype(c) == icol_sunwall .or. col%itype(c) == icol_shadewall &
               ...
               if (j >= snl(c)+1 .AND. j <= nlevurb-1) then
                 ...
                 .and. col%itype(c) /= icol_roof) then
               ! Finn Windfly for soil thermal conductivity parameterization
               !if (j >= snl(c)+1 .AND. j <= nlevgrnd-1) then
               !   tk(c,j) = thk(c,j)*thk(c,j+1)*(z(c,j+1)-z(c,j)) &
               !        /(thk(c,j)*(z(c,j+1)-zi(c,j))+thk(c,j+1)*(zi(c,j)-z(c,j)))
               if (j >= snl(c)+1 .AND. j <= 0) then 
                  tk(c,j) = thk(c,j)*thk(c,j+1)*(z(c,j+1)-z(c,j)) &
                       /(thk(c,j)*(z(c,j+1)-zi(c,j))+thk(c,j+1)*(zi(c,j)-z(c,j)))
               else if (j >= 1 .AND. j <=nlevgrnd-1) then
                  mu1 = (soilstate_inst%cellsand_col(c,j+1)+soilstate_inst%cellorg_col(c,j+1))/ &
                        ((soilstate_inst%cellsand_col(c,j)+soilstate_inst%cellsand_col(c,j+1))+(soilstate_inst%cellorg_col(c,j)+soilstate_inst%cellorg_col(c,j+1))) 
                  mu2 = (soilstate_inst%cellsand_col(c,j)+soilstate_inst%cellorg_col(c,j))/ &
                        ((soilstate_inst%cellsand_col(c,j)+soilstate_inst%cellsand_col(c,j+1))+(soilstate_inst%cellorg_col(c,j)+soilstate_inst%cellorg_col(c,j+1)))
                  tk(c,j) = 2._r8*mu1*mu2*thk(c,j)*thk(c,j+1)*(z(c,j+1)-z(c,j)) &
                       /(mu1*thk(c,j)*(z(c,j+1)-zi(c,j))+mu2*thk(c,j+1)*(zi(c,j)-z(c,j)))
