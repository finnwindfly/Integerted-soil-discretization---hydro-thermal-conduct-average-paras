...
!scs: vangenuchten
     ...
     ! Finn add dynamic soil porosity
     real(r8), pointer :: ori_dz               (:,:)
     real(r8), pointer :: ori_z                (:,:)
     real(r8), pointer :: ori_zi               (:,:)
...
contains
  ...
  subroutine InitAllocate(this, bounds)
    ...
    !Finn add for dynamic soil porosity
    allocate(this%ori_dz               (begc:endc,nlevgrnd))            ; this%ori_dz               (:,:) = nan
    allocate(this%ori_z                (begc:endc,nlevgrnd))            ; this%ori_z                (:,:) = nan
    allocate(this%ori_zi               (begc:endc,0:nlevgrnd))          ; this%ori_zi               (:,:) = nan
...
!-----add variables putting in history file-----by-Finn_Windfly
    this%cellorg_col(begc:endc,:) = spval
    call hist_addfld2d (fname='ORGANIC_COL',  units='%', type2d='levsoi',  &
         avgflag='A', long_name='organic fraction in soil layer', &
         ptr_col=this%cellorg_col)

    this%cellsand_col(begc:endc,:) = spval
    call hist_addfld2d (fname='SAND_COL',  units='%', type2d='levsoi',  &
         avgflag='A', long_name='percent of sand in soil layer', &
         ptr_col=this%cellsand_col)

    this%cellclay_col(begc:endc,:) = spval
    call hist_addfld2d (fname='CLAY_COL',  units='%', type2d='levsoi',  &
         avgflag='A', long_name='percent of clay in soil layer', &
         ptr_col=this%cellclay_col)

    this%bd_col(begc:endc,:) = spval
    call hist_addfld2d (fname='BD_COL',  units='Kg/m^3', type2d='levgrnd',  &
         avgflag='A', long_name='bulk density in soil layer', &
         ptr_col=this%bd_col)

    this%sucsat_col(begc:endc,:) = spval
    call hist_addfld2d (fname='SUCSAT_COL',  units='mm', type2d='levgrnd',  &
         avgflag='A', long_name='minimum soil suction (natural vegetated and crop landunits only)', &
         ptr_col=this%sucsat_col, set_spec=spval, l2g_scale_type='veg')

    this%hk_l_col(begc:endc,:) = spval
    call hist_addfld2d (fname='HK_COL',  units='mm/s', type2d='levgrnd',  &
         avgflag='A', long_name='hydraulic conductivity (natural vegetated and crop landunits only)', &
         ptr_col=this%hk_l_col, set_spec=spval, l2g_scale_type='veg')

    this%hksat_col(begc:endc,:) = spval
    call hist_addfld2d (fname='HKSAT_COL',  units='mm/s', type2d='levgrnd',  &
         avgflag='A', long_name='hydraulic conductivity at saturation(natural vegetated and crop landunits only)', &
         ptr_col=this%hksat_col, set_spec=spval, l2g_scale_type='veg')

    this%thk_col(begc:endc,0:nlevgrnd) = spval
    data2dptr => this%thk_col(:,0:nlevgrnd)
    call hist_addfld2d (fname='TKSOIL_COL', units='W/m-K', type2d='levgrnd', &
         avgflag='A', long_name='Thermal conductivity of soil layers', &
         ptr_col=data2dptr)

    this%tksatu_col(begc:endc,:) = spval
    call hist_addfld2d (fname='TKSAT_COL',  units='W/m-K', type2d='levgrnd',  &
         avgflag='A', long_name='Saturated thermal conductivity of soil layers', &
         ptr_col=this%tksatu_col)

    this%csol_col(begc:endc,:) = spval
    call hist_addfld2d (fname='CSOL_COL',  units='J/m^3/K', type2d='levgrnd',  &
         avgflag='A', long_name='soil heat capacity', &
         ptr_col=this%csol_col)

    data2dptr => this%ori_dz(begc:endc,1:nlevsoi)
    call hist_addfld2d (fname='ORIDZ',  units='m', type2d='levsoi', &
         avgflag='A', long_name='dynamic soil thickness(vegetated landunits only)',&
         ptr_col=data2dptr, l2g_scale_type='veg')

    data2dptr => this%ori_z(begc:endc,1:nlevsoi)
    call hist_addfld2d (fname='ORIZ',  units='m', type2d='levsoi', &
         avgflag='A', long_name='dynamic soil layer depth(vegetated landunits only)',&
         ptr_col=data2dptr, l2g_scale_type='veg')

    data2dptr => this%ori_zi(begc:endc,1:nlevsoi)
    call hist_addfld2d (fname='ORIZI',  units='m', type2d='levsoi', &
         avgflag='A', long_name='dynamic soil interface depth(vegetated landunits only)',&
         ptr_col=data2dptr, l2g_scale_type='veg')
...
!----ends here-------------------------------------------------
  ...
  subroutine InitCold(this, bounds)
    ...
    !-----------------------------------------------------------
    ! Finn add for initilize dyn_dz equal col%dz
    !-----------------------------------------------------------
    do c = bounds%begc, bounds%endc
       this%ori_zi(c,0) = 0._r8
       do j = 1, nlevgrnd
          this%ori_dz(c,j) = col%dz(c,j)
          this%ori_dz(c,j) = col%dz(c,j)
          !this%dyn_watsat(c,j) = 0.48_r8
          this%ori_zi(c,j) = col%zi(c,j)
          this%ori_z(c,j)  = col%z(c,j)
       end do
    end do
    ! Finn's initilation ends here
