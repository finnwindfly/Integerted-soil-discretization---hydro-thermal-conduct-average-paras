...
contains
  ...
  subroutine SoilStateInitTimeConst(bounds, soilstate_inst, nlfilename) 
    ...
    !Finn's modification for local variables definations
    real(r8)           :: sand_obs(40)
    real(r8)           :: clay_obs(40)
    real(r8)           :: organic_obs(40)
    real(r8)           :: varphi1
    real(r8)           :: varphi2
    integer            :: user_defined_soil = 1
    !-----------------------------------------------------------------------
    ...
    do c = begc, endc
       ...
       if (lun%itype(l)==istwet .or. lun%itype(l)==istice_mec .or. &
          ...
          do lev = 1,nlevgrnd
             ...
             ! Finn Windfly for soil texture parameterization
             if (user_defined_soil == 1) then
               sand_obs = (/40.87,35.87,29.94,32.02,34.09,41.63,49.08,48.94,52.89,60.84,68.78,73.46,78.13,91.6,77.5,50.91, &
                            50.91,50.91,50.91,50.91,50.91,50.91,50.91,50.91,50.91,50.91,50.91,50.91,50.91,50.91,50.91,50.91, &
                            50.91,50.91,50.91,50.91,50.91,50.91,50.91,50.91/)
               clay_obs = (/13.56,8.56,11.02,9.93,8.894,10.57,7.99,9.33,7.34,6.39,5.43,4.66,3.89,1.54,4.29,9.71, &
                            9.71,9.71,9.71,9.71,9.71,9.71,9.71,9.71,9.71,9.71,9.71,9.71,9.71,9.71,9.71,9.71, &
                            9.71,9.71,9.71,9.71,9.71,9.71,9.71,9.71/)      
               organic_obs=(/130.00,130.00,86.72,74.12,61.51,23.99,21.23,14.24,14.02,13.79,10.92,8.05,3.85,2.65,2.46,2.80, &
                            2.80,2.80,2.80,2.80,2.80,2.80,2.80,2.80,2.80,2.80,2.80,2.80,2.80,2.80,2.80,2.80, &
                            2.80,2.80,2.80,2.80,2.80,2.80,2.80,2.80/)        
               sand = sand_obs(lev)
               clay = clay_obs(lev)
               om_frac = organic_obs(lev)/organic_max
             else
               if (lev .eq. 1) then
                  clay = clay3d(g,1)
                  sand = sand3d(g,1)
             ...
             if (lun%itype(l) /= istdlak) then  ! soil columns of both urban and non-urban types
                ...
                !Finn Windfly for update saturated hydraulic conductivity for mineral
                if (lev <= nlevsoi .and. lev > 1) then
                   varphi1 = soilstate_inst%cellsand_col(c,lev) / (soilstate_inst%cellsand_col(c,lev)+soilstate_inst%cellsand_col(c,lev-1))
                   varphi2 = soilstate_inst%cellsand_col(c,lev-1) / (soilstate_inst%cellsand_col(c,lev)+soilstate_inst%cellsand_col(c,lev-1))
                   soilstate_inst%hksat_min_col(c,lev) = soilstate_inst%hksat_min_col(c,lev-1)*varphi1 + soilstate_inst%hksat_min_col(c,lev)*varphi2
                end if
