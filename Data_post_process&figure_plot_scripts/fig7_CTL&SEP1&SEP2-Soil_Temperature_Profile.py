...
def read_observed_temperature(csv_file, start_date_str, end_date_str, depths):
    ...
    # 删除2月29日的数据
    leap_day_mask = ~((temperatures.index.month == 2) & (temperatures.index.day == 29))
    temperatures = temperatures[leap_day_mask]
    
    print(f"观测数据时间范围: {temperatures.index.min()} 到 {temperatures.index.max()}")
    print(f"观测数据长度: {len(temperatures)}")
    
    return temperatures.T

def plot_temperature_profile(ax, time_grid, depth_grid, data, label, site_index, col_index, color_norm, ylim, ctl_time):
    """绘制单个土壤温度剖面图"""
    # 绘制等值线填充图
    c = ax.contourf(time_grid, depth_grid, data, levels=20, cmap=cmaps.BlueYellowRed, norm=color_norm, extend='both')
...
def plot_all_profiles(obs_data, ctl_data, sep1_data, sep2_data,
    ...
    # 打印调试信息
    print(f"\n站点 {site_index + 1} 时间信息:")
    print(f"起始日期: {start_date}")
    print(f"结束日期: {end_date}")
    print(f"CTL数据时间范围: {ctl_data.time.values[0]} 到 {ctl_data.time.values[-1]}")
    print(f"CTL数据长度: {len(ctl_data.time)}")
    ...
    print(f"处理后观测时间长度: {len(time_obs)}")
    print(f"处理后模拟时间长度: {len(time_sim)}")
    ...
    for i in range(4):
        ...
        plot_temperature_profile(ax[site_index, i], time_grids[i], depth_grids[i], 
...
if __name__ == "__main__":
    ...
    plt.tight_layout()
    plt.savefig(f'{base_path}/fig/202506/fig7_Soil_Temperature_Profile_CTL&SEP1&SEP2_nobedrock.jpg',
