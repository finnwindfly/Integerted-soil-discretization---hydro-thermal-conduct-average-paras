...
def read_nc_temperature(nc_file, depths, start_date_str, end_date_str):
    ...
    # 对每个深度进行插值并返回列表
    return [temp.sel(time=time_slice).interp(levgrnd=depth/100.0) for depth in depths]
...
def plot_temperature_comparison(ax, sim_data_old, sim_data_new, obs_data, depths, dates):
    """绘制温度对比图并计算TS评分"""
    ...
    for i, depth in enumerate(depths):
        ...
        # 添加TS评分文本
        ax[row, col].text(0.05, 0.9, f'TS = {ts_old:.3f}', transform=ax[row, col].transAxes,
                        color=colors[0], fontsize=14)
        ax[row, col].text(0.05, 0.8, f'TS = {ts_new:.3f}', transform=ax[row, col].transAxes,
        ...
        # 设置y轴标签和刻度
        if col == 0:
            ax[row, col].set_ylabel('Temperature (°C)', fontsize=14)
        ax[row, col].yaxis.set_major_formatter(ticker.FormatStrFormatter('%.1f'))
        ...
        if i == 0:
            ax[row, col].legend(frameon=False, loc='lower right', fontsize=14)
...
if __name__ == "__main__":
    ...
    # 绘图
    plot_temperature_comparison(ax, sim_data_old, sim_data_new, obs_data, depths, 
                            [pd.to_datetime(d) for d in dates])
    
    plt.tight_layout()
    plt.savefig(f'{output_path}/fig10_old&new_scheme-Soil_Temperature-restart.jpg', bbox_inches='tight', dpi=300)
    plt.show()
